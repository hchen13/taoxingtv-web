...
var require_agent_src = __commonJS({
  "agent-src.js"() {
    ...
    function dumpList(listVal, ChannelCls) {
      if (!listVal) return [];
      const n = listVal.size();
      const out = [];
      for (let i = 0; i < n; i++) {
        const ch = frida_java_bridge_default.cast(listVal.get(i), ChannelCls);
        const g = (f) => {
          try {
            const v = ch[f].value;
    ...
    rpc.exports = {
      ...
      dump: function() {
        return new Promise((resolve, reject) => frida_java_bridge_default.perform(function() {
          try {
            const CD = frida_java_bridge_default.use("com.wys.iptvgo.coredata.ChannelData");
            const Channel = frida_java_bridge_default.use("com.wys.iptvgo.coredata.Channel");
            resolve({
              live: dumpList(CD.listChannel.value, Channel),
      ...
      probeStream: function(port, maxMs) {
        return new Promise((resolve, reject) => frida_java_bridge_default.perform(function() {
          try {
            ...
            const conn = frida_java_bridge_default.cast(URL.$new("http://127.0.0.1:" + port + "/").openConnection(), frida_java_bridge_default.use("java.net.HttpURLConnection"));
            conn.setConnectTimeout(5e3);
            conn.setReadTimeout(12e3);
            const is = conn.getInputStream();
            const buf = frida_java_bridge_default.array("byte", Array(65536).fill(0));
            ...
            resolve({ bytes: total, ms: Date.now() - t0, eof });
          } catch (e) {
            reject("" + (e.stack || e));
          }
        }));
      ...
      play: function(chid, startTime) {
        return new Promise((resolve, reject) => frida_java_bridge_default.perform(function() {
          ...
          try {
            step = "stopPrev";
            try {
              frida_java_bridge_default.use("dnet.VideoClient").playbackStop();
            } catch (e) {
            }
            step = "cb";
            const cb = ensureCB();
      ...
      engineReady: function() {
        return new Promise((resolve) => frida_java_bridge_default.perform(function() {
          try {
            ...
            let at = 0;
            try {
              at = parseInt("" + CD.activatedTime.value, 10) || 0;
            } catch (e) {
            }
            let ch = 0;
            try {
            ...
            resolve({ activated: false, activatedTime: 0, channels: 0, err: "" + (e.message || e) });
          }
        }));
      },
      readCreds: function() {
      ...
      saveCreds: function(account, password) {
        return new Promise((resolve, reject) => frida_java_bridge_default.perform(function() {
          try {
            const ctx = frida_java_bridge_default.use("android.app.ActivityThread").currentApplication();
            const SU = frida_java_bridge_default.use("com.common.util.SettingUtil");
            const DH = frida_java_bridge_default.use("com.common.util.DeviceHelper");
